...
export function startWorker(name: QueueName, processor: Processor): Worker {
  const worker = new Worker(name, processor, { connection: getConnection(), concurrency: 4 });
  worker.on('error', (err) => {
    // Redis provider limits can surface as Worker errors; report them without
    // crashing the process.
    // eslint-disable-next-line no-console
    console.error('[worker] worker error', name, err.message);
  });
  return worker;
}
